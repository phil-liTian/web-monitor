...
function isError(error) {
    return ['[object Error]', '[object Exception]', '[object DOMException]'].includes(Object.prototype.toString.call(error));
}
 ...
 * @returns
 */
function replaceAop(source, name, callback, isForce = false) {
    if (!source)
        return;
...
function setSilentFlag({ slientError = true, slientClick = true, slientFetch = true, slientXhr = true, slientUnhanderRejection = true, slientHashChange = true, slientHistoryChange = true, silentWhiteScreen = true }) {
...
class Breadcrumb {
    ...
    bindOptions(options) {
        ...
        validateOption(boforePushBreadcrumb, 'boforePushBreadcrumb', 'function') &&
            (this.boforePushBreadcrumb = boforePushBreadcrumb);
    }
    getCategory(type) {
        switch (type) {
            // js异常
...
class TransportData {
    ...
    getTransportData(data) {
        const info = Object.assign(Object.assign(Object.assign({}, data), this.getAuthInfo()), { pageUrl: getLocationHref(), deviceInfo: _support.deviceInfo });
...
class Options {
    constructor() {
        this.overTime = 10; // 超时事件, 默认是10s
        this.throttleDelayTime = 0;
        this.whiteBoxElements = [];
        this.skeletonProject = false;
        this.repeatCodeError = true;
    }
    bindOptions(options) {
        const { overTime, throttleDelayTime, skeletonProject, repeatCodeError, filterXhrUrlRegExp, whiteBoxElements = ['html', 'body', '#app', '#root'], } = options;
...
function handleOptions(parmasOptions) {
 ...
 * @author: phil.li
 */
function log({ message = 'customMsg', error, type = EVENTTYPES.CUSTOM, }) {
    try {
        let errorInfo = {};
        if (isError(error)) {
            const result = ErrorStackParser.parse(!error.target ? error : error.error || error.reason)[0];
            errorInfo = Object.assign(Object.assign({}, result), { line: result.lineNumber, column: result.columnNumber });
        }
        breadcrumb.push({
            type,
        ...
        transportData.send(Object.assign({ type, status: STATUSCODE.ERROR, message: unknownToString(message), time: getTimestamp() }, errorInfo));
    }
    catch (error) {
        console.log(error);
    }
...
function notify(type, data) {
    ...
    if (!type || !handlers[type])
        return;
    (_a = handlers[type]) === null || _a === void 0 ? void 0 : _a.forEach(callback => {
        try {
            callback(data);
        }
        catch (error) {
...
function listenUnhandledrejection() {
    on(_global, EVENTTYPES.UNHANDLEDREJECTION, (e) => {
        // 阻止控制台报红色错误
        // e.preventDefault();
 ...
 */
function openWhiteScreen(callback, { skeletonProject, whiteBoxElements }) {
    let _whiteLoopNum = 0;
...
const HandleEvents = {
    ...
    handleError(event) {
        ...
        if (!target || (target && !target.localName)) {
            ...
            const hash = getErrorUid(`${EVENTTYPES.ERROR}-${event.message}-${fileName}-${lineNumber}-${columnNumber}`);
        ...
        if (target === null || target === void 0 ? void 0 : target.localName) {
            ...
            transportData.send(Object.assign(Object.assign({}, data), { type: EVENTTYPES.RESOURCE, status: STATUSCODE.ERROR }));
        }
    },
    ...
    handleUnhandledrejection(ev) {
        ...
        const hash = getErrorUid(`${EVENTTYPES.UNHANDLEDREJECTION}-${message}-${fileName}-${lineNumber}-${columnNumber}`);
        // 控制重复错误不上报, 如果repeatCodeError为true, 则控制重复错误不上报
        if (!options.repeatCodeError || (options.repeatCodeError && !hashMapExist(hash))) {
    ...
    handleWhiteScreen() {
        openWhiteScreen(res => {
            transportData.send(Object.assign({ type: EVENTTYPES.WHITESCREEN, time: getTimestamp() }, (res || {})));
        }, options);
    },
};
